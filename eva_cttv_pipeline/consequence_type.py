...
class ConsequenceType(object):
    class SoTerm(object):
        ...
        def __init__(self, so_name):
            ...
            if so_name in ConsequenceType.SoTerm.so_accession_name_dict:
                self._so_accession = ConsequenceType.SoTerm.so_accession_name_dict[so_name]
            else:
        ...
        def get_rank(self):
            ...
            if self.get_name() not in ConsequenceType.SoTerm.ranked_so_names_list:
                return len(ConsequenceType.SoTerm.ranked_so_names_list)
            else:
                return ConsequenceType.SoTerm.ranked_so_names_list.index(self.get_name())

        @staticmethod
        def get_ranked_so_names():
            return ConsequenceType.SoTerm.ranked_so_names_list
    ...
    def __init__(self, ensembl_gene_ids=None, so_names=None):
        ...
        if so_names is not None:
            self._so_terms = set([ConsequenceType.SoTerm(so_name) for so_name in so_names])
        else:
            self._so_terms = None
    ...
    def add_so_term(self, soName):
        if self._so_terms is None:
            self._so_terms = {ConsequenceType.SoTerm(soName)}
        else:
            self._so_terms.add(ConsequenceType.SoTerm(soName))
